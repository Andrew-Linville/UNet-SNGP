...
class GPHeadMulti(nn.Module):
    """
    - Train beta with standard CE/BCE loss (beta is in model parameters).
    - After training, call:
      ...
      For K>1, this uses a diagonal-block Laplace approx:
        Σ_inv[k] += Z^T diag(p_k(1-p_k)) Z    (ignores cross-class Fisher terms)
    - At inference, variance_from_Z(Z) computes per-class logit variance.
    """
    ...
    def reset_precision(self, tau=1e-3):
        ...
        eye = torch.eye(M, device=self.beta.device)
    ...
    def accumulate_precision(self, Z, logits):
        ...
        if self.num_classes == 1:
            # Binary (single-logit) case: Bernoulli likelihood
    ...
    def variance_from_Z(self, Z):
        ...
        for k in range(K):
            # Solve Σ_inv[k] * X = Z^T  via Cholesky → X = Σ * Z^T
